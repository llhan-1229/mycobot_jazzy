...
场景中有两根同尺寸圆柱：红柱中心为 `(0.22, 0.12, 0.175)`，蓝柱中心为
`(0.12, 0.22, 0.175)`。两者到 `base_link` 的水平半径约为 `0.2506 m`，
放置位置共用现有 `place_pose`。

通过 `target_color:=red` 或 `target_color:=blue` 指定抓取目标，默认抓取红色：
...
颜色来自对齐深度点云的 RGB 字段，聚类点逐点转换到 HSV。默认颜色阈值为：
`color_min_saturation: 0.4`、`color_min_value: 0.1`、`color_min_confidence: 0.6`。
感知先严格匹配颜色，再进行形状和尺寸筛选；目标颜色不存在、颜色未知或置信度不足
时服务失败，不会回退到另一种颜色。两根圆柱都会加入 PlanningScene，未选中的圆柱
继续作为碰撞障碍物。
